...
(function(p5) {
   ...
   * @param initialCapacity
   */
  const create$g = initialCapacity => {
    return {
      array: new Array(initialCapacity),
      size: 0
    };
   ...
   * @returns `true` if `queue.size === queue.array.length`.
   */
  const isFull$1 = queue => queue.size === queue.array.length;
  /**
   * Adds `element` to `queue` as the last (newest) element.
   * Be sure that `queue` is not full.
   * @param queue
   ...
   * @param element
   */
  const enqueueSafe$1 = (queue, element) => {
    if (!isFull$1(queue)) enqueue$1(queue, element);
  };
  /**
   * Removes the top (oldest) element from `queue`.
   * Be sure that `queue` is not empty.
   ...
   * @returns Removed element, or `undefined` if not full.
   */
  const dequeueIfFull$1 = queue =>
    isFull$1(queue) ? dequeue$1(queue) : undefined;
  /**
   * Retunrs the top (oldest) element from `queue`.
   * Be sure that `queue` is not empty.
   * @param queue
  ...
  const dequeueSafeIf$1 = (queue, predicate) =>
    isEmpty$1(queue) ? undefined : dequeueIf$1(queue, predicate);
  /**
   * Clears the contents of `queue`.
   * This does not nullify references.
   * @param queue
  ...
  const fullName$2$1 = /*#__PURE__*/ Object.freeze({
    ...
    arrayQueueIsFull: isFull$1,
    enqueue: enqueue$1,
    enqueueSafe: enqueueSafe$1,
    dequeue: dequeue$1,
    dequeueSafe: dequeueSafe$1,
    dequeueIfFull: dequeueIfFull$1,
   ...
   * @returns new `Vector2D`.
   */
  const normalizeBetween$2 = (from, to) => {
    const dx = to.x - from.x;
    const dy = to.y - from.y;
   ...
   * @param minDistance
   */
  const separate$1 = (vector, from, minDistance) => {
   ...
   * @param midPointRatio The ratio for determining the midpoint from `a` to `b`.
   */
  const separateEachOther$1 = (a, b, minDistance, midPointRatio) => {
  ...
  const mutable$1 = /*#__PURE__*/ Object.freeze({
    ...
    separate: separate$1,
    separateEachOther: separateEachOther$1
  });
  ...
  const normalize$1$1 = (vector, target) => {
    const { x, y } = vector;
    const length = hypotenuse2D$1(x, y);
    target.x = x / length;
    target.y = y / length;
  ...
  const normalizeBetween$1$1 = (from, to, target) => {
    const dx = to.x - from.x;
    const dy = to.y - from.y;
    const distance = hypotenuse2D$1(dx, dy);
    target.x = dx / distance;
  ...
  const assign$1 = /*#__PURE__*/ Object.freeze({
    ...
    normalizeBetween: normalizeBetween$1$1
  });
  ...
  const fullName$3$1 = /*#__PURE__*/ Object.freeze({
    ...
    separateVector: separate$1,
    separateVectors: separateEachOther$1,
  ...
  const index$3$1 = /*#__PURE__*/ Object.freeze({
    ...
    normalizeBetween: normalizeBetween$2
  });
   ...
   * @param component
   */
  const reset$2 = component => component.reset();
  ...
  class Unit$1$1 extends Base$1 {
    ...
    reset() {
      loop$4(this.components, reset$2);
      setIndex$1(this, 0);
      this.isStarted = false;
      this.isCompleted = false;
      return this;
  ...
  class Unit$2$1 extends Base$1 {
    ...
    reset() {
      ...
      loop$1$1(runningComponentList, reset$2);
      this.isStarted = false;
      this.isCompleted = false;
      return this;
    }
  ...
  const create$a$1 = capacity => {
    return {
      runningComponents: create$g(capacity),
      newComponentsBuffer: create$g(capacity)
    };
  };
  ...
  const run$2 = channel => {
    ...
    channel.index += 1;
    if (channel.index < channel.signals.length) {
      setNextRun$1(run$2, channel, currentSignalTimeLength);
      return;
    }
    channel.timeout = setTimeout(() => {
      if (channel.loop) {
        ...
        setNextRun$1(run$2, channel, MEDIUM_GAP$1.length);
      }
    }, currentSignalTimeLength);
    channel.index = 0;
  };
  ...
  const repeater$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    create: create$e$1,
    run: run$1$1,
    reset: reset$1$1
   ...
   * @returns The `quantity` instance with assigned force.
   */
  const addForce$2 = (quantity, force) => {
    quantity.fx += force.x;
    quantity.fy += force.y;
    return quantity;
  };
   ...
   * @param forceOnBodyB
   */
  const addForceEachOther$2 = (bodyA, bodyB, forceOnBodyB) => {
    const { x: forceX, y: forceY } = forceOnBodyB;
    bodyA.fx -= forceX;
    bodyA.fy -= forceY;
    bodyB.fx += forceX;
   ...
   * @returns The `target` vector with assigned values.
   */
  const calculateForce$1 = (vx, vy, normalUnitVector, restitution, target) => {
    const dotProduct = -(vx * normalUnitVector.x + vy * normalUnitVector.y);
   ...
   * @param restitution
   */
  const withinRectangle$1 = (quantity, region, restitution) => {
    const { x, y } = quantity;
    const {
      topLeft: { x: minX, y: minY },
      bottomRight: { x: maxX, y: maxY }
  ...
  /** A temporal vector for storing the impulsive force by the bounce. */
  const bounceForce$1 = create$4$2();
  /**
   * Calculates and adds the impulsive force by the bounce.
   * @param quantity
   * @param normalUnitVector
  ...
  const normalUnitVector$1 = create$4$2();
  /**
   * Calculates and adds the impulsive force by the bounce.
   * Note that the penetration will not be fixed.
   */
  const addForceEachOther$1$1 = {
    calculate: (quantityA, quantityB, restitution) => {
      const bRelativeVelocityX = quantityB.vx - quantityA.vx;
      const bRelativeVelocityY = quantityB.vy - quantityA.vy;
      normalizeBetween$1$1(quantityA, quantityB, normalUnitVector$1);
      calculateForce$1(
        bRelativeVelocityX,
        bRelativeVelocityY,
        normalUnitVector$1,
      ...
      addForce$2(quantityB, bounceForce$1);
      multiply$1$1(bounceForce$1, -1);
      addForce$2(quantityA, bounceForce$1);
    },
    preCalculated: (
      quantityA,
      quantityB,
   ...
   * @param restitution
   */
  const addForceWithinRectangle$1 = (quantity, region, restitution) => {
    const { x, y } = quantity;
    const {
      topLeft: { x: minX, y: minY },
      bottomRight: { x: maxX, y: maxY }
  ...
  const bounce$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    calculateForce: calculateForce$1,
    withinRectangle: withinRectangle$1,
    addForce: addForce$1$1,
    addForceEachOther: addForceEachOther$1$1,
    addForceWithinRectangle: addForceWithinRectangle$1
  });
