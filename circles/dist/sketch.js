...
(function () {
    ...
    const sketch = (p) => {
        ...
        function initialize() {
            ...
            for (let i = 0; i < 16; i++) {
                if (Math.random() < 0.5) {
                    const hueValue = (baseHue + p.random(-1, 1) * 0.3 * p.TWO_PI) % p.TWO_PI;
                    const circleColor = cielchColor(100, 100, hueValue);
                ...
                p.ellipse(baseX + p.random(-1, 1) * 70, baseY + p.random(-1, 1) * 70, diameter, diameter);
            }
