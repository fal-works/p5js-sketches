/**
 * InfoPollution.
 * Source code in TypeScript: https://github.com/fal-works/p5js-sketches/tree/master/info-pollution
 *
 * Bundled libraries:
...
(function(p5) {
   ...
   */
  const { Random } = CCC;
  const { onSetup: onSetup$1, ShapeColor, setRenderer: setRenderer$1 } = p5ex;
  /**
   * Shared p5 instance.
   */
  let p$1;
  ...
  const Letter = (() => {
    ...
    const createDraw = (value, polluted) => {
      ...
      return (x, y) => p$1.image(graphics, x, y);
    };
    const create = (x, y, value, polluted = false) => {
      const width = p$1.textWidth(value);
      const isSpace = value === " ";
      const draw = !isSpace ? createDraw(value, polluted) : emptyFunction;
      return {
        value,
        width,
        isSpace,
    ...
    const isAlive = letter => letter.isAlive;
    const exists = letter => !!letter;
    return {
      create,
      update,
      draw,
  ...
  const createLetter = character => {
    ...
    if (x > 0.75 * canvas$1.logicalSize.width) {
      breakLine();
    }
    return letter;
  };
  ...
  const getSubsequentLetters = (letter, letterList) =>
    letterList.filter(
  ...
  const initialize = () => {
    ...
    const initialTextWidth = p$1.textWidth(testString);
  ...
  const drawTextCursor = () => {
    p$1.stroke(0, 128 + 127 * Math.sin(p$1.frameCount * 0.1));
    p$1.strokeWeight(2);
    p$1.line(x, y, x, y + fontSize);
  };
  const drawSketch = () => {
