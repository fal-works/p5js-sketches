...
(function(p5) {
   ...
   * @param resolution
   */
  const create$e = (color, resolution) => {
  ...
  const overwrite = (shapeColor, strokeColor, fillColor, alphaResolution) => {
    ...
    if (strokeColor === null) {
      ...
      const strokeAlphaColor = create$e(strokeColor, alphaResolution);
    ...
    if (fillColor === null) {
      ...
      const fillAlphaColor = create$e(fillColor, alphaResolution);
   ...
   * @returns A new array.
   */
  const unifyToArray$2 = arrayOrValue =>
    arrayOrValue
      ? Array.isArray(arrayOrValue)
        ? arrayOrValue.slice()
        : [arrayOrValue]
   ...
   * @returns A new number array.
   */
  const createIntegerSequence$1 = length => {
    const array = new Array(length);
    for (let i = 0; i < length; i += 1) array[i] = i;
    return array;
  };
   ...
   * @returns New array, filtered and mapped.
   */
  const filterMap$1 = (array, callback) => {
    const result = [];
    const len = array.length;
    for (let i = 0; i < len; i += 1) {
      const mappedValue = callback(array[i], i, array);
   ...
   * @param argument
   */
  const loopRunWithArgument$1 = (functionArray, argument) => {
    const len = functionArray.length;
    for (let i = 0; i < len; i += 1) functionArray[i](argument);
  };
  /**
  ...
  const fullName$7 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    loopArrayRange: loopRange$1,
    loopArray: loop$4,
    loopArrayRangeBackwards: loopRangeBackwards$1,
    loopArrayBackwards: loopBackwards$2,
    arrayNestedLoopJoinWithRange: nestedLoopJoinWithRange$1,
    arrayNestedLoopJoin: nestedLoopJoin$2,
    arrayRoundRobinWithRange: roundRobinWithRange$1,
    arrayRoundRobin: roundRobin$2,
    unifyToArray: unifyToArray$2,
    flatArrayNaive: flatNaive$1,
    flatArrayRecursive: flatRecursive$1,
    populateArray: populate$2,
    createPopulatedArray: createPopulated$2,
    createIntegerSequence: createIntegerSequence$1,
    arrayFromRange: fromRange$1,
    filterMapArray: filterMap$1,
    loopRunArray: loopRun$1,
    loopRunArrayWithArgument: loopRunWithArgument$1,
    blitArray: blit$1
  });
  ...
  const index$a = /*#__PURE__*/ Object.freeze({
    ...
    loopRun: loopRun$1,
    loopRunWithArgument: loopRunWithArgument$1,
    blit: blit$1
  });
   ...
   * @param initialCapacity
   */
  const create$f = initialCapacity => {
    return {
      array: new Array(initialCapacity),
      size: 0
    };
   ...
   * @returns The element of `arrayList` at `index`.
   */
  const get$4 = (arrayList, index) => arrayList.array[index];
  /**
   * Returns the last element of `arrayList`.
   * Be sure that `arrayList` is not empty.
   * @param arrayList
   * @returns The last element of `arrayList`.
   */
  const peek$2 = arrayList => arrayList.array[arrayList.size - 1];
  /**
   * Returns the last element of `arrayList`.
   * Be sure that `arrayList` is not empty.
   * Same as `peek()`.
   ...
   * @returns The last element of `arrayList`.
   */
  const getLast$1 = peek$2;
  /**
   * Adds all elements of `array` to `arrayList`.
   * @param arrayList
   * @param array
   */
  const addArray$1 = (arrayList, array) => {
    const { array: thisArray, size: destinaionPosition } = arrayList;
    const len = array.length;
    let i = len;
    while (i) {
   ...
   * @param source
   */
  const addList$1 = (destination, source) => {
    const { array: destinationArray, size: destinaionPosition } = destination;
    const { array: sourceArray, size: len } = source;
    let i = len;
    while (i) {
   ...
   * @param arrayList
   */
  const clearReference$2 = arrayList => {
    arrayList.size = 0;
  ...
  const fullName$1$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    createArrayList: create$f,
    createFilledArrayList: createFilled$1,
    createPopulatedArrayList: createPopulated$1$1,
    arrayListFromArray: fromArray$1,
    addToArrayList: add$5,
    pushToArrayList: push$1,
    popFromArrayList: pop$1,
    getFromArrayList: get$4,
    peekFromArrayList: peek$2,
    getLastFromArrayList: getLast$1,
    addArrayToArrayList: addArray$1,
    addListToArrayList: addList$1,
    clearArrayList: clear$4,
    cleanUnusedSlotsOfArrayList: cleanUnusedSlots$1,
    clearReferenceOfArrayList: clearReference$2,
    loopArrayList: loop$1$1,
    loopArrayListBackwards: loopBackwards$1$1,
    findInArrayList: find$1,
    findIndexInArrayList: findIndex$1,
    removeShiftFromArrayList: removeShift$1,
    removeShiftElementFromArrayList: removeShiftElement$1,
    removeSwapFromArrayList: removeSwap$1,
    removeSwapElementFromArrayList: removeSwapElement$1,
    removeShiftAllFromArrayList: removeShiftAll$1,
    removeSwapAllFromArrayList: removeSwapAll$1,
    populateArrayList: populate$1$1,
    arrayListNestedLoopJoin: nestedLoopJoin$1$1,
    arrayListRoundRobin: roundRobin$1$1
  });

  const index$1$2 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    ...
    clearReference: clearReference$2,
    loop: loop$1$1,
   ...
   * @returns `true` if `queue.size === 0`.
   */
  const isEmpty$1 = queue => queue.size === 0;
  /**
  ...
  const enqueueSafe = (queue, element) => {
    if (!isFull(queue)) enqueue$1(queue, element);
  };
  /**
   * Removes the top (oldest) element from `queue`.
   * Be sure that `queue` is not empty.
   ...
   * @returns Removed element, or `undefined` if empty.
   */
  const dequeueSafe$1 = queue =>
    isEmpty$1(queue) ? undefined : dequeue$1(queue);
  /**
   * Removes the top (oldest) element from `queue` only if `queue` is full.
   * @param queue
   * @returns Removed element, or `undefined` if not full.
   */
  const dequeueIfFull = queue => (isFull(queue) ? dequeue$1(queue) : undefined);
  /**
   * Retunrs the top (oldest) element from `queue`.
   * Be sure that `queue` is not empty.
   * @param queue
   * @returns Removed element.
   */
  const peek$1$1 = queue => queue.array[queue.headIndex];
  /**
   * Retunrs the top (oldest) element from `queue`.
   * @param queue
   * @returns Removed element, or `undefined` if empty.
   */
  const peekSafe$1 = queue => {
    const { headIndex } = queue;
    return headIndex !== queue.tailIndex ? queue.array[headIndex] : undefined;
  };
  /**
   ...
   * @param callback
   */
  const loop$2$1 = (queue, callback) => {
    if (queue.size === 0) return;
    const { array, headIndex, tailIndex } = queue;
    if (headIndex < tailIndex) {
      loopRange$1(array, callback, headIndex, tailIndex);
      return;
    }
    loopRange$1(array, callback, headIndex, array.length);
    loopRange$1(array, callback, 0, tailIndex);
  };
  /**
   * Removes the top (oldest) element from `queue` if `predicate` returns true.
   * Be sure that `queue` is not empty.
   ...
   * @returns Removed element, or `undefined` if not removed.
   */
  const dequeueIf$1 = (queue, predicate) => {
    const { array, headIndex } = queue;
    const topElement = array[headIndex];
    if (!predicate(topElement)) return undefined;
    const nextHeadIndex = headIndex + 1;
   ...
   * @returns Removed element, or `undefined` if empty or not removed.
   */
  const dequeueSafeIf$1 = (queue, predicate) =>
    queue.headIndex !== queue.tailIndex
      ? dequeueIf$1(queue, predicate)
      : undefined;
  /**
   * Clears the contents of `queue`.
   * This does not nullify references.
   ...
   * @param queue
   */
  const clearReference$1$1 = queue => {
    clear$1$1(queue);
    const { array } = queue;
    const capacity = array.length;
    array.length = 0;
  ...
  const fullName$2$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    createArrayQueue: create$1$2,
    arrayQueueIsEmpty: isEmpty$1,
    arrayQueueIsFull: isFull,
    enqueue: enqueue$1,
    enqueueSafe: enqueueSafe,
    dequeue: dequeue$1,
    dequeueSafe: dequeueSafe$1,
    dequeueIfFull: dequeueIfFull,
    loopArrayQueue: loop$2$1,
    peekArrayQueue: peek$1$1,
    peekArrayQueueSafe: peekSafe$1,
    dequeueIf: dequeueIf$1,
    dequeueSafeIf: dequeueSafeIf$1,
    clearArrayQueue: clear$1$1,
    clearArrayQueueReference: clearReference$1$1
  });
  ...
  const index$2$1 = /*#__PURE__*/ Object.freeze({
    ...
    dequeueSafeIf: dequeueSafeIf$1,
    clear: clear$1$1,
    clearReference: clearReference$1$1
  });

  const create$2$2 = factory => {
    return {
      value: undefined,
      factory
    };
  };
  const get$1$1 = object => object.value || (object.value = object.factory());
  const clear$2$1 = object => {
    object.value = undefined;
  };
  ...
  const square$2 = v => v * v;
  const cube$1 = v => v * v * v;
  const pow4$1 = v => square$2(v * v);
  const pow5$1 = v => square$2(v * v) * v;
   ...
   * @returns Interpolated value, e.g. `start` if `ratio == 0`, `end` if `ratio == 1`.
   */
  const lerp$2 = (start, end, ratio) => start + ratio * (end - start);
  /**
   * Clamps `value` between `min` and `max`.
   * @param value
   * @param min
   ...
   * @returns Constrained value.
   */
  const constrain$3 = (value, min, max) =>
    min > max ? (min + max) / 2 : value < min ? min : value > max ? max : value;
  /**
   * Maps `value` from the range [`inStart`, `inEnd`] to the range [`outStart`, `outEnd`].
   * @param value
   ...
   * @returns Mapped value (unclamped).
   */
  const map$1 = (value, inStart, inEnd, outStart, outEnd) =>
    outStart + ((outEnd - outStart) * (value - inStart)) / (inEnd - inStart);
  /**
   * Creates a mapping function that maps `value` from the range [`inStart`, `inEnd`] to the range [`outStart`, `outEnd`].
   * @param inStart
   ...
   * @returns New mapping function.
   */
  const createMap$1 = (inStart, inEnd, outStart, outEnd) => {
    const inLength = inEnd - inStart;
    const outLength = outEnd - outStart;
    return value => outStart + (outLength * (value - inStart)) / inLength;
  };
   ...
   * @returns Mapped value between 0 and 1 (unclamped).
   */
  const inverseLerp$2 = (value, start, end) => (value - start) / (end - start);
  ...
  const angle$5 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    PI: PI$1,
    TWO_PI: TWO_PI$2,
    HALF_PI: HALF_PI$1,
   ...
   * @returns Square of distance.
   */
  const distanceSquared$2 = (vectorA, vectorB) =>
   ...
   * @returns Distance.
   */
  const distance$3 = (vectorA, vectorB) =>
  ...
  const normalize = vector => {
    const { x, y } = vector;
    const length = hypotenuse2D$1(x, y);
    return {
      x: x / length,
      y: y / length
    };
  ...
  const normalizeBetween = (from, to) => {
    const dx = to.x - from.x;
    const dy = to.y - from.y;
    const distance = hypotenuse2D$1(dx, dy);
    return {
      x: dx / distance,
      y: dy / distance
    };
  ...
  const setCartesian$3 = (vector, x, y) => {
    vector.x = x;
    vector.y = y;
    return vector;
  };
  ...
  const clamp$1$1 = (vector, minX, maxX, minY, maxY) => {
    vector.x = clamp$4(vector.x, minX, maxX);
    vector.y = clamp$4(vector.y, minY, maxY);
    return vector;
  };
  const constrain$1$1 = (vector, minX, maxX, minY, maxY) => {
    vector.x = constrain$3(vector.x, minX, maxX);
    vector.y = constrain$3(vector.y, minY, maxY);
    return vector;
  };
  /**
  ...
  const separate = (vector, from, minDistance) => {
    const distanceSquared$1 = distanceSquared$2(vector, from);
    if (distanceSquared$1 >= minDistance * minDistance) return vector;
    const angle$1 = betweenPoints$1(from, vector);
    vector.x = from.x + minDistance * cos$2(angle$1);
    vector.y = from.y + minDistance * sin$2(angle$1);
    return vector;
  };
  /**
  ...
  const separateEachOther = (a, b, minDistance, midPointRatio) => {
    const distanceSquared$1 = distanceSquared$2(a, b);
    if (distanceSquared$1 >= minDistance * minDistance) return;
    const angleFromA = betweenPoints$1(a, b);
    const midX = lerp$2(a.x, b.x, midPointRatio);
    const midY = lerp$2(a.y, b.y, midPointRatio);
    const halfMinDistance = minDistance / 2;
    const bDisplacementX = halfMinDistance * cos$2(angleFromA);
    const bDisplacementY = halfMinDistance * sin$2(angleFromA);
  ...
  const mutable$1 = /*#__PURE__*/ Object.freeze({
    ...
    set: set$2,
    setCartesian: setCartesian$3,
    setPolar: setPolar$2,
    multiply: multiply$1$1,
    divide: divide$1$1,
    clamp: clamp$1$1,
    constrain: constrain$1$1,
    separate: separate,
  ...
  const clamp$2$1 = (vector, minX, maxX, minY, maxY, target) => {
    target.x = clamp$4(vector.x, minX, maxX);
    target.y = clamp$4(vector.y, minY, maxY);
    return target;
  };
  const constrain$2$1 = (vector, minX, maxX, minY, maxY, target) => {
    target.x = constrain$3(vector.x, minX, maxX);
    target.y = constrain$3(vector.y, minY, maxY);
    return target;
  };
  ...
  const assign$1 = /*#__PURE__*/ Object.freeze({
    ...
    divide: divide$2$1,
    clamp: clamp$2$1,
    constrain: constrain$2$1,
    normalize: normalize$1,
    normalizeBetween: normalizeBetween$1
  });

  const fullName$3$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    createVector: create$3$2,
    zeroVector: zero$1,
    vectorIsZero: isZero$1,
    vectorFromPolar: fromPolar$1,
    addVector: add$1$1,
    addCartesian: addCartesian$3,
    addPolar: addPolar$3,
    subtractVector: subtract$3,
    subtractCartesian: subtractCartesian$3,
    subtractPolar: subtractPolar$3,
    multiplyVector: multiply$3,
    divideVector: divide$3,
    distanceOfVectorsSquared: distanceSquared$2,
    distanceOfVectors: distance$3,
    vectorToStr: toStr$1,
    copyVector: copy$2,
    vectorLengthSquared: lengthSquared$1,
    vectorLength: length$1,
    vectorAngle: angle$1$1,
    createVectorMutable: create$4$2,
    addVectorMutable: add$2$1,
    addCartesianMutable: addCartesian$1$1,
    addPolarMutable: addPolar$1$1,
    subtractVectorMutable: subtract$1$1,
    subtractCartesianMutable: subtractCartesian$1$1,
    subtractPolarMutable: subtractPolar$1$1,
    setVector: set$2,
    setCartesian: setCartesian$3,
    setPolar: setPolar$2,
    multiplyVectorMutable: multiply$1$1,
    divideVectorMutable: divide$1$1,
    clampVector: clamp$1$1,
    constrainVector: constrain$1$1,
    separateVector: separate,
    separateVectors: separateEachOther,
    addVectorAssign: add$3$1,
    addCartesianAssign: addCartesian$2$1,
    addPolarAssign: addPolar$2$1,
    subtractVectorAssign: subtract$2$1,
    subtractCartesianAssign: subtractCartesian$2$1,
    subtractPolarAssign: subtractPolar$2$1,
    multiplyVectorAssign: multiply$2$1,
    divideVectorAssign: divide$2$1,
    clampVectorAssign: clamp$2$1,
    constrainVectorAssign: constrain$2$1
  });

  const index$3$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    Mutable: mutable$1,
    Assign: assign$1,
    FullName: fullName$3$1,
    create: create$3$2,
    zero: zero$1,
    isZero: isZero$1,
  ...
  const create$5$2 = (topLeftPosition, size) => ({
    topLeft: topLeftPosition,
    bottomRight: {
      x: topLeftPosition.x + size.width,
      y: topLeftPosition.y + size.height
  ...
  const createFromCenter$1 = (centerPosition, size) => {
    const { x, y } = centerPosition;
    const halfWidth = size.width / 2;
    const halfHeight = size.height / 2;
    return {
  ...
  const getWidth$1 = region => region.bottomRight.x - region.topLeft.x;
  const getHeight$1 = region => region.bottomRight.y - region.topLeft.y;
    ...
    switch (originType) {
      case 0:
        return {
          topLeft,
          bottomRight: {
            x: lerp$2(topLeft.x, bottomRight.x, scaleFactor),
            y: lerp$2(topLeft.y, bottomRight.y, scaleFactor)
          }
        };
      case 1: {
   ...
   * @returns The cloned instance.
   */
  const copy$1$1 = region => ({
    topLeft: copy$2(region.topLeft),
    bottomRight: copy$2(region.bottomRight)
  });
  /**
   * @returns A `RectangleRegion` instance with `Infinity` values.
   */
  const createInfinite$2 = () => ({
    topLeft: { x: -Infinity, y: -Infinity },
    bottomRight: { x: Infinity, y: Infinity }
  });
  /**
   ...
   * @returns A new `RectangleRegion` unit.
   */
  const addMargin$1 = (region, margin) => {
    const {
      topLeft: originalTopLeft,
      bottomRight: originalBottomRight
    } = region;
   ...
   * @returns A new `RectangleRegion` unit.
   */
  const addMargins$1 = (region, margins) => {
    const {
      topLeft: originalTopLeft,
      bottomRight: originalBottomRight
    } = region;
  ...
  const rectangleRegion$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    create: create$5$2,
    createFromCenter: createFromCenter$1,
    ...
    copy: copy$1$1,
    createInfinite: createInfinite$2,
    addMargin: addMargin$1,
    addMargins: addMargins$1
  });
  ...
  const rectangleSize$1 = /*#__PURE__*/ Object.freeze({
    ...
    getArea: getArea$1,
    createInfinite: createInfinite$1$1
  });
   ...
   * @returns `dx^2 + dy^2`.
   */
  const distanceSquared$1$1 = (x1, y1, x2, y2) =>
    hypotenuseSquared2D$1(x2 - x1, y2 - y1);
  /**
   * Calculates the distance between [`x1`, `y1`] and [`x2`, `y2`];
   * @param x1
   * @param y1
   ...
   * @returns `√(dx^2 + dy^2)`.
   */
  const distance$1$1 = (x1, y1, x2, y2) => hypotenuse2D$1(x2 - x1, y2 - y1);

  const coordinates2d$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    distanceSquared: distanceSquared$1$1,
   ...
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const quad$4 = square$2;
  /**
   * "easeInCubic" function.
   * @param x Any ratio.
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const cubic$4 = cube$1;
  /**
   * "easeInQuart" function.
   * @param x Any ratio.
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const quart$4 = pow4$1;
  /**
   * "easeInExpo" function.
   * @param x Any ratio.
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const expo$4 = x => (x ? pow$1(2, 10 * (x - 1)) : 0);
  /**
   * Creates a new "easeInBack" function with `coefficient`.
   * @param coefficient Defaults to 1.70158
   * @returns "easeInBack" function.
   */
  const createBack$4 = (coefficient = 1.70158) => x =>
    x * x * ((coefficient + 1) * x - coefficient);

  const _in$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    quad: quad$4,
    cubic: cubic$4,
    quart: quart$4,
    expo: expo$4,
    createBack: createBack$4
  });
   ...
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const quad$1$1 = x => -square$2(x - 1) + 1;
  /**
   * "easeOutCubic" function.
   * @param x Any ratio.
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const cubic$1$1 = x => cube$1(x - 1) + 1;
  /**
   * "easeOutQuart" function.
   * @param x Any ratio.
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const quart$1$1 = x => -pow4$1(x - 1) + 1;
  /**
   * "easeOutExpo" function.
   * @param x Any ratio.
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const expo$1$1 = x => (x < 1 ? -pow$1(2, -10 * x) + 1 : 1);
  /**
   * Creates a new "easeOutBack" function with `coefficient`.
   * @param coefficient Defaults to 1.70158
   * @returns "easeOutBack" function.
   */
  const createBack$1$1 = (coefficient = 1.70158) => {
    return x => {
      const r = x - 1;
      const r2 = r * r;
      return (coefficient + 1) * (r * r2) + coefficient * r2 + 1;
  ...
  const out$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    quad: quad$1$1,
    cubic: cubic$1$1,
    quart: quart$1$1,
    expo: expo$1$1,
    createBack: createBack$1$1
  });
   ...
   * @returns New easing function.
   */
  const concatenate$1 = (easingA, easingB, thresholdRatio = 0.5) => {
    const inverseThresholdRatio = 1 / thresholdRatio;
    return ratio => {
      if (ratio < thresholdRatio) return easingA(inverseThresholdRatio * ratio);
      else {
   ...
   * @returns New easing function.
   */
  const integrate$1 = (easingA, easingB, thresholdRatio = 0.5) => {
    const inverseThresholdRatio = 1 / thresholdRatio;
    return ratio => {
      if (ratio < thresholdRatio)
        return thresholdRatio * easingA(inverseThresholdRatio * ratio);
   ...
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const quad$2$1 = integrate$1(quad$4, quad$1$1);
  /**
   * "easeInOutCubic" function.
   * @param x Any ratio.
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const cubic$2$1 = integrate$1(cubic$4, cubic$1$1);
  /**
   * "easeInOutQuart" function.
   * @param x Any ratio.
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const quart$2$1 = integrate$1(quart$4, quart$1$1);
  /**
   * "easeInOutExpo" function.
   * @param x Any ratio.
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const expo$2$1 = integrate$1(expo$4, expo$1$1);
  /**
   * Creates a new "easeInOutBack" function with `coefficient`.
   * @param coefficient Defaults to 1.70158
   * @returns "easeInOutBack" function.
   */
  const createBack$2$1 = coefficient =>
    integrate$1(createBack$4(coefficient), createBack$1$1(coefficient));

  const inOut$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    quad: quad$2$1,
    cubic: cubic$2$1,
    quart: quart$2$1,
    expo: expo$2$1,
    createBack: createBack$2$1
  });
   ...
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const quad$3$1 = integrate$1(quad$1$1, quad$4);
  /**
   * "easeOutInCubic" function.
   * @param x Any ratio.
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const cubic$3$1 = integrate$1(cubic$1$1, cubic$4);
  /**
   * "easeOutInQuart" function.
   * @param x Any ratio.
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const quart$3$1 = integrate$1(quart$1$1, quart$4);
  /**
   * "easeOutInExpo" function.
   * @param x Any ratio.
   * @returns Eased ratio. `0` if x=0, `1` if x=1.
   */
  const expo$3$1 = integrate$1(expo$1$1, expo$4);
  /**
   * Creates a new "easeOutInBack" function with `coefficient`.
   * @param coefficient Defaults to 1.70158
   * @returns "easeOutInBack" function.
   */
  const createBack$3$1 = coefficient =>
    integrate$1(createBack$1$1(coefficient), createBack$4(coefficient));

  const outIn$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    quad: quad$3$1,
    cubic: cubic$3$1,
    quart: quart$3$1,
    expo: expo$3$1,
    createBack: createBack$3$1
  });
  ...
  const fullName$4$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    easeInQuad: quad$4,
    easeInCubic: cubic$4,
    easeInQuart: quart$4,
    easeInExpo: expo$4,
    createEaseInBack: createBack$4,
    easeOutQuad: quad$1$1,
    easeOutCubic: cubic$1$1,
    easeOutQuart: quart$1$1,
    easeOutExpo: expo$1$1,
    createEaseOutBack: createBack$1$1,
    easeInOutQuad: quad$2$1,
    easeInOutCubic: cubic$2$1,
    easeInOutQuart: quart$2$1,
    easeInOutExpo: expo$2$1,
    createEaseInOutBack: createBack$2$1,
    easeOutInQuad: quad$3$1,
    easeOutInCubic: cubic$3$1,
    easeOutInQuart: quart$3$1,
    easeOutInExpo: expo$3$1,
    createEaseOutInBack: createBack$3$1,
    easeLinear: linear$1,
    concatenateEasing: concatenate$1,
    integrateEasing: integrate$1
  });

  const index$4$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    In: _in$1,
    Out: out$1,
    InOut: inOut$1,
    OutIn: outIn$1,
    FullName: fullName$4$1,
    linear: linear$1,
   ...
   * @returns A random value.
   */
  let ratio$3 = random$1;
  /**
   * Sets `randomFunction` as the base function (which is initially set to `Math.random`)
   * so that it will be used as the base of all `Random` functions.
   * @param randomFunction - Any function that returns a (pseudo-)random number from `0` up to (but not including) `1`.
   */
  const setBaseFunction$1 = randomFunction => {
    random$1 = randomFunction;
    ratio$3 = randomFunction;
  };
   ...
   * @returns A random value.
   */
  const value$4 = max => random$1() * max;
  /**
   * Returns random value from `0` to (but not including) `2 * PI`.
   * @returns A random radians value.
   */
  const angle$2$1 = () => random$1() * TWO_PI$2;
  /**
   * Returns random value from `start` up to (but not including) `end`.
   * @param start
   * @param end
   * @returns A random value.
   */
  const between$4 = (start, end) => start + random$1() * (end - start);
  /**
   * Returns random value from `range.start` up to (but not including) `range.end`.
   * @param range
   * @returns A random value.
   */
  const inRange$2 = range => between$4(range.start, range.end);
  /**
   * Returns `true` or `false` randomly.
   * @param probability A number between 0 and 1.
   * @returns `true` with the given `probability`.
   */
  const bool$1 = probability => random$1() < probability;
  /**
   * Returns `1` or `-1` randomly.
   * @param positiveProbability A number between 0 and 1 for the probability of a positive value being returned.
   * @returns Either `1` or `-1`.
   */
  const sign$1$1 = positiveProbability =>
    random$1() < positiveProbability ? 1 : -1;
  /**
   * Returns a positive or negative value randomly with a magnitude from `0` up to (but not including) `maxMagnitude`.
   * @param maxMagnitude
   * @returns A random value.
   */
  const signed$4 = maxMagnitude =>
    (random$1() < 0.5 ? 1 : -1) * random$1() * maxMagnitude;
  /**
   * Returns a positive or negative value randomly with a magnitude from `0` up to (but not including) `PI`.
   * @returns A random radians value.
   */
  const signedAngle$3 = () => (random$1() < 0.5 ? 1 : -1) * random$1() * PI$1;
  ...
  const pointInRectangleRegion$1 = region => {
    const { topLeft, bottomRight } = region;
    return {
      x: between$4(topLeft.x, bottomRight.x),
      y: between$4(topLeft.y, bottomRight.y)
    };
  };
   ...
   * @returns A random integer value.
   */
  const value$1$1 = maxInt => floor$1(random$1() * maxInt);
  /**
   * Returns random integer from `minInt` up to (but not including) `maxInt`.
   * The case where `minInt > maxInt` or `maxInt <= 0` is not expected.
   * @param minInt
   ...
   * @returns A random integer value.
   */
  const between$1$1 = (minInt, maxInt) =>
    minInt + floor$1(random$1() * (maxInt - minInt));
  /**
   * Returns a positive or negative integer randomly
   * with a magnitude from `0` up to (but not including) `maxMagnitude`.
   * @param maxMagnitude
   * @returns A random signed value.
   */
  const signed$1$1 = maxMagnitude =>
    (random$1() < 0.5 ? 1 : -1) * floor$1(random$1() * maxMagnitude);

  const integer$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    value: value$1$1,
    between: between$1$1,
    signed: signed$1$1
   ...
   * @returns A random value.
   */
  const ratio$1$1 = step => floor$1(random$1() / step) * step;
  /**
   * Returns a random value at intervals of `step` from `0` up to (but not including) `max`.
   * @param step
   * @param max
   * @returns A random value.
   */
  const value$2$1 = (step, max) => floor$1(random$1() * (max / step)) * step;
  /**
   * Returns a random value at intervals of `step` from `min` up to (but not including) `max`.
   * @param step
   * @param min
   ...
   * @returns A random value.
   */
  const between$2$1 = (step, min, max) =>
    min + floor$1(random$1() * ((max - min) / step)) * step;
  /**
   * Returns a positive or negative value randomly at intervals of `step`
   * with a magnitude from `0` up to (but not including) `maxMagnitude`.
   * @param step
   ...
   * @returns A random signed value.
   */
  const signed$2$1 = (step, maxMagnitude) =>
    (random$1() < 0.5 ? 1 : -1) *
    floor$1(random$1() * (maxMagnitude / step)) *
    step;
  /**
   * Returns a random value at intervals of `step` from `0` to (but not including) `2 * PI`.
   * @param step - Interval angle.
   * @returns A random radians value.
   */
  const angle$3$1 = step => floor$1(random$1() * (TWO_PI$2 / step)) * step;
  /**
   * Returns a positive or negative value randomly at intervals of `step`
   * with a magnitude from `0` up to (but not including) `PI`.
   * @param step - Interval angle.
   * @returns A random signed radians value.
   */
  const signedAngle$1$1 = step =>
    (random$1() < 0.5 ? 1 : -1) * floor$1(random$1() * (PI$1 / step)) * step;

  const discrete$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    ratio: ratio$1$1,
    value: value$2$1,
    between: between$2$1,
   ...
   * @returns A random element.
   */
  const get$2$1 = array => array[value$1$1(array.length)];
  /**
   * Removes and returns one element from `array` randomly.
   * `array` is not expected to be empty.
   * @param array
   * @returns A random element.
   */
  const removeGet$1 = array => array.splice(value$1$1(array.length), 1)[0];

  const arrays$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    get: get$2$1,
    removeGet: removeGet$1
  });
   ...
   * @returns A random value.
   */
  const ratio$2$1 = curve => curve(random$1());
  /**
   * Similar to `value()`, but remaps the result by `curve`.
   * @param curve Function that takes a random value between [0, 1] and returns a remapped value.
   * @param magnitude
   * @returns A random value.
   */
  const value$3$1 = (curve, magnitude) => curve(random$1()) * magnitude;
  /**
   * Similar to `between()`, but remaps the result by `curve`.
   * @param curve Function that takes a random value between [0, 1] and returns a remapped value.
   * @param start
   ...
   * @returns A random value.
   */
  const between$3$1 = (curve, start, end) =>
    start + curve(random$1()) * (end - start);
  /**
   * Similar to `inRange()`, but remaps the result by `curve`.
   * @param curve Function that takes a random value between [0, 1] and returns a remapped value.
   * @param range
   * @returns A random value.
   */
  const inRange$1$1 = (curve, range) =>
    between$3$1(curve, range.start, range.end);
  /**
   * Similar to the normal `angle()`, but remaps the result by `curve`.
   * @param curve Any function that takes a random value between [0, 1) and returns a remapped value.
   * @returns A random radians value.
   */
  const angle$4$1 = curve => curve(random$1()) * TWO_PI$2;
  /**
   * Similar to the normal `signed()`, but remaps the result by `curve`.
   * @param curve Any function that takes a random value between [0, 1) and returns a remapped value.
   * @param magnitude
   * @returns A random signed value.
   */
  const signed$3$1 = (curve, magnitude) =>
    (random$1() < 0.5 ? 1 : -1) * curve(random$1()) * magnitude;
  /**
   * Similar to the normal `signedAngle()`, but remaps the result by `curve`.
   * @param curve Any function that takes a random value between [0, 1) and returns a remapped value.
   * @param magnitude
   * @returns A random signed radians value.
   */
  const signedAngle$2$1 = curve =>
    (random$1() < 0.5 ? 1 : -1) * curve(random$1()) * PI$1;

  const curved$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    ratio: ratio$2$1,
    value: value$3$1,
    between: between$3$1,
    ...
    angle: angle$4$1,
    signed: signed$3$1,
    signedAngle: signedAngle$2$1
  });

  const fullName$5$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    setBaseRandomFunction: setBaseFunction$1,
    get randomRatio() {
      return ratio$3;
    },
    randomValue: value$4,
    randomAngle: angle$2$1,
    randomBetween: between$4,
    randomInRange: inRange$2,
    randomBool: bool$1,
    randomSign: sign$1$1,
    randomSigned: signed$4,
    randomSignedAngle: signedAngle$3,
    randomInteger: value$1$1,
    randomIntegerBetween: between$1$1,
    randomIntegerSigned: signed$1$1,
    randomDiscreteRatio: ratio$1$1,
    randomDiscreteValue: value$2$1,
    randomDiscreteAngle: angle$3$1,
    randomDiscreteBetween: between$2$1,
    randomDiscreteSigned: signed$2$1,
    randomDiscreteSignedAngle: signedAngle$1$1,
    randomFromArray: get$2$1,
    randomRemoveFromArray: removeGet$1,
    randomRatioCurved: ratio$2$1,
    randomValueCurved: value$3$1,
    randomBetweenCurved: between$3$1,
    randomInRangeCurved: inRange$1$1,
    randomVector: vector$1,
    randomPointInRectangleRegin: pointInRectangleRegion$1
  });

  const index$5$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    Integer: integer$1,
    Discrete: discrete$1,
    Arrays: arrays$1,
    Curved: curved$1,
    FullName: fullName$5$1,
    get random() {
      return random$1;
    },
    get ratio() {
    ...
    setBaseFunction: setBaseFunction$1,
    value: value$4,
    angle: angle$2$1,
    between: between$4,
    inRange: inRange$2,
  ...
  const returnVoid$2 = () => {};
  const returnUndefined$1 = () => undefined;
  const returnNull$1 = () => null;
  const returnZero$1 = () => 0;
  const returnOne$1 = () => 1;
  const returnTrue$1 = () => true;
  const returnFalse$1 = () => false;
  const returnArgument$1 = argument => argument;
  /**
   * Runs `callback` without any arguments.
   * @param callback - Any function that can be run without any arguments.
   */
  const runSelf$1 = callback => callback();

  const constantFunction$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    returnVoid: returnVoid$2,
    returnUndefined: returnUndefined$1,
    returnNull: returnNull$1,
    returnZero: returnZero$1,
    returnOne: returnOne$1,
    returnTrue: returnTrue$1,
    returnFalse: returnFalse$1,
    returnArgument: returnArgument$1,
    runSelf: runSelf$1
  });

  let verbose$1 = returnVoid$2;
  const outputVerbose$1 = (yes = true) => {
    verbose$1 = yes ? console.info : returnVoid$2;
  };
  const TIMER$1 = "timer ";
  const CREATED$1 = ": created.";
  const STARTING$1 = ": starting...";
  const STARTED$1 = ": started.";
  const COMPLETING$1 = ": completing...";
  const COMPLETED$1 = ": completed.";

  let nextComponentId$1 = 0;
  /**
   * Callback function for running `component.step()`.
   * @param component
   */
  const step$2 = component => component.step();
  /**
   * Callback function for running `component.reset()`.
   * @param component
   */
  const reset$1 = component => component.reset();
  const defaultName$1 = "no name";
  /**
   * Base class for other classes implementing `Component`.
   */
  class Base$1 {
    constructor(onStart, onComplete) {
      this.id = nextComponentId$1++;
      this.name = defaultName$1;
      this.onStart = onStart;
      this.onComplete = onComplete;
      this.isStarted = false;
      this.isCompleted = false;
      verbose$1(TIMER$1, this.id, CREATED$1);
    }
    /**
     * If `this` is not yet started,
     * runs all functions in `this.onStart`, and sets `this.isStarted` to `true`.
    ...
    tryStart() {
      if (this.isStarted) return false;
      const { id, name } = this;
      verbose$1(TIMER$1, id, name, STARTING$1);
      loopRunWithArgument$1(this.onStart, id);
      verbose$1(TIMER$1, id, name, STARTED$1);
      return (this.isStarted = true);
    }
    /**
     * Runs all functions in `this.onComplete`, and sets `this.isCompleted` to `true`.
    ...
    complete() {
      const { id, name } = this;
      verbose$1(TIMER$1, id, name, COMPLETING$1);
      loopRunWithArgument$1(this.onComplete, id);
      verbose$1(TIMER$1, id, name, COMPLETED$1);
      return (this.isCompleted = true);
    }
    /**
     * Sets the name of `this` for debug purpose.
  ...
  const component$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    step: step$2,
    reset: reset$1,
    Base: Base$1
  ...
  const createProgress$1 = duration => {
    return {
      duration,
      ratioChangeRate: 1 / max2$2(1, duration),
      count: 0,
      ratio: 0
    };
  };
  const updateProgress$1 = progress => {
    progress.count += 1;
    progress.ratio += progress.ratioChangeRate;
  };
  const resetProgress$1 = progress => {
    progress.count = 0;
    progress.ratio = 0;
  };
  class Unit$4 extends Base$1 {
    constructor(onStart, onProgress, onComplete, progress) {
      super(onStart, onComplete);
      this.onProgress = onProgress;
      this.progress = progress;
    ...
    static create(onStart, onProgress, onComplete, progress) {
      return new Unit$4(onStart, onProgress, onComplete, progress);
    }
    step() {
      if (this.isCompleted) return true;
      this.tryStart();
      ...
      if (progress.count >= progress.duration) {
        progress.ratio = 1;
        loopRunWithArgument$1(this.onProgress, progress);
        return this.complete();
      }
      loopRunWithArgument$1(this.onProgress, progress);
   ...
   * @returns New `Timer` instance.
   */
  const create$6$2 = parameters => {
    return Unit$4.create(
      unifyToArray$2(parameters.onStart),
      unifyToArray$2(parameters.onProgress),
      unifyToArray$2(parameters.onComplete),
      createProgress$1(parameters.duration)
    );
  };
  const dummy$1 = Unit$4.create([], [], [], createProgress$1(0));
  dummy$1.isStarted = true;
  dummy$1.isCompleted = true;

  const setIndex$1 = (chain, index) => {
    chain.index = index;
    chain.currentComponent = chain.components[index];
  };
  /**
   ...
   * @returns `true` if completed i.e. there is no next component.
   */
  const setNextIndex$1 = chain => {
    const nextIndex = chain.index + 1;
    if (nextIndex < chain.components.length) {
      setIndex$1(chain, nextIndex);
      return false;
    }
    return chain.complete();
  };
  class Unit$1$1 extends Base$1 {
    constructor(components) {
      super([], []);
      this.components = components.slice();
      this.index = 0;
    ...
    static create(components) {
      return new Unit$1$1(components);
    }
    step() {
      this.tryStart();
      if (!this.currentComponent.step()) return false;
      return setNextIndex$1(this);
    }
    reset() {
      loop$4(this.components, reset$1);
      setIndex$1(this, 0);
      this.isStarted = false;
      this.isCompleted = false;
      return this;
   ...
   * @returns New `Timer.Chain` instance.
   */
  const create$7$2 = components => Unit$1$1.create(components);
  ...
  class Unit$2$1 extends Base$1 {
    ...
    static create(components) {
      return new Unit$2$1(components);
    }
    step() {
      this.tryStart();
      const { runningComponentList } = this;
      removeShiftAll$1(runningComponentList, step$2);
      if (runningComponentList.size > 0) return false;
      return this.complete();
    }
    reset() {
      const { runningComponentList } = this;
      clear$4(runningComponentList);
      addArray$1(runningComponentList, this.components);
      loop$1$1(runningComponentList, reset$1);
      this.isStarted = false;
      this.isCompleted = false;
      return this;
    }
    ...
    addComponent(component) {
      this.components.push(component);
      add$5(this.runningComponentList, component);
    }
  }
  /**
   * Creates a parallel composite from `components`.
   ...
   * @returns New `Timer.Parallel` instance.
   */
  const create$8$1 = components => Unit$2$1.create(components);

  const parallel$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    Unit: Unit$2$1,
    create: create$8$1
  });

  class Unit$3$1 extends Base$1 {
    constructor(component, loopCount) {
      super([], []);
      this.component = component;
      this.loopCount = loopCount;
    ...
    static create(component, loopCount) {
      return new Unit$3$1(component, loopCount);
    }
    step() {
      this.tryStart();
      if (!this.component.step()) return false;
   ...
   * @returns New `Timer.Loop` instance.
   */
  const create$9$1 = (component, loopCount = Infinity) =>
    Unit$3$1.create(component, loopCount);

  const loop$3$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    Unit: Unit$3$1,
    create: create$9$1
  });

  const create$a$1 = capacity => {
    return {
      runningComponents: create$f(capacity),
      newComponentsBuffer: create$f(capacity)
    };
  };
  const add$4$1 = (timerSet, component) =>
    add$5(timerSet.newComponentsBuffer, component);
  const step$1$1 = timerSet => {
    const { runningComponents, newComponentsBuffer } = timerSet;
    addList$1(runningComponents, newComponentsBuffer);
    clear$4(newComponentsBuffer);
    removeShiftAll$1(runningComponents, step$2);
  };
  const clear$3$1 = timerSet => {
    clear$4(timerSet.runningComponents);
    clear$4(timerSet.newComponentsBuffer);
  };
  /**
   * Creates a timer set instance and returns a set of bound functions.
   * @param capacity
  ...
  const construct$1 = capacity => {
    const timerSet = create$a$1(capacity);
    return {
      add: add$4$1.bind(undefined, timerSet),
      step: step$1$1.bind(undefined, timerSet),
      clear: clear$3$1.bind(undefined, timerSet)
    };
  };
  ...
  const index$6$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
  ...
  const createSignalUnit$1 = (isOn, length, codeString) => {
    let s = "";
    const binaryCharacter = isOn ? "1" : "0";
    for (let i = 0; i < length; i += 1) {
      s += binaryCharacter;
  ...
  const createOnSignalUnit$1 = (length, codeString) =>
    createSignalUnit$1(true, length, codeString);
  const createOffSignalUnit$1 = (length, codeString) =>
    createSignalUnit$1(false, length, codeString);
  const DIT$1 = createOnSignalUnit$1(1, ".");
  const DAH$1 = createOnSignalUnit$1(3, "-");
  const INTER_ELEMENT_GAP$1 = createOffSignalUnit$1(1, "");
  const SHORT_GAP$1 = createOffSignalUnit$1(3, " ");
  const MEDIUM_GAP$1 = createOffSignalUnit$1(7, " / ");
  const NUL$1 = createOffSignalUnit$1(0, "");
  ...
  const create$b$1 = (on, off, wpm = 25, signals = [], loop = false) => {
    return {
      on,
      off,
      wpm,
  ...
  const index$7$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    Signal: signal$1,
    Channel: channel$1
  });
   ...
   * @returns New `Timer` instance.
   */
  const create$c$1 = (setValue, duration, parameters) => {
    const { start, end } = parameters;
    const ease = parameters.easing || linear$1;
    return create$6$2({
      duration,
      onProgress: progress => setValue(lerp$2(start, end, ease(progress.ratio)))
    });
  };
  /**
   * Creates a `Timer` instance for tweening value using `setValue()`.
   ...
   * @returns New `Timer` instance.
   */
  const setCreate$2 = (setValue, duration, getParameters) => {
    let startValue;
    let endValue;
    let ease;
    return create$6$2({
      ...
      onStart: () => {
        ...
        startValue = start;
        endValue = end;
        ease = easing || linear$1;
      },
      onProgress: progress =>
        setValue(lerp$2(startValue, endValue, ease(progress.ratio)))
    });
  };
   ...
   * @returns New `Timer` instance.
   */
  const create$d$1 = (vector, duration, parameters) => {
    const { x: startX, y: startY } = vector;
    const { x: endX, y: endY } = parameters.target;
    const ease = parameters.easing || linear$1;
    return create$6$2({
      ...
      onProgress: progress => {
        ...
        setCartesian$3(
          vector,
          lerp$2(startX, endX, ratio),
          lerp$2(startY, endY, ratio)
        );
      }
    });
  };
   ...
   * @returns New `Timer` instance.
   */
  const setCreate$1$1 = (vector, duration, getParameters) => {
    let startX, startY;
    let endX, endY;
    let ease;
    return create$6$2({
  ...
  const vector2d$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    create: create$d$1,
    setCreate: setCreate$1$1
  });

  const index$8$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    Vector2D: vector2d$1,
    create: create$c$1,
    setCreate: setCreate$2
  });
   ...
   */
  const create$e$1 = (callback, frequency = 1) => ({
    callback,
    frequency,
    accumulation: 0
  });
   ...
   * Prints info message.
   */
  let info$1 = returnVoid$2;
  /**
   * Prints warning message.
   */
  let warn$1 = returnVoid$2;
  /**
   * Prints error message.
   */
  let error$1 = returnVoid$2;
  /**
   * Sets if info messages should be output to console log.
   * @param yes
   */
  const outputInfo$1 = (yes = true) => {
    info$1 = yes ? console.info : returnVoid$2;
  };
  /**
   * Sets if warning messages should be output to console log.
   * @param yes
   */
  const outputWarn$1 = (yes = true) => {
    warn$1 = yes ? console.warn : returnVoid$2;
  };
  /**
   * Sets if error messages should be output to console log.
   * @param yes
   */
  const outputError$1 = (yes = true) => {
    error$1 = yes ? console.error : returnVoid$2;
  };

  const log$1$1 = /*#__PURE__*/ Object.freeze({
    __proto__: null,
    get info() {
      return info$1;
    },
    ...
    outputWarn: outputWarn$1,
    outputError: outputError$1
  });
  ...
  const calculate$1 = (attractor, attracted, target) =>
    calculateCore$1(
      ...
      attractor.mass * attracted.mass,
      distance$3(attractor, attracted),
      target
    );
  /**
   * Calculates gravitation force, assuming that the mass is always `1`.
  ...
  const createRandomQuantity$1 = (minRotationSpeed, maxRotationSpeed) => {
    return {
      ...
      rotationVelocity: signed$4(between$4(minRotationSpeed, maxRotationSpeed))
    };
  };
  ...
  const calculateForce = (vx, vy, normalUnitVector, restitution, target) => {
    ...
    multiply$2$1(normalUnitVector, (1 + restitution) * dotProduct, target);
    return target;
  };
  /**
   ...
   * @param restitution
   */
  const addForce$1$1 = (quantity, normalUnitVector, restitution) =>
    addForce$1(
      quantity,
      calculateForce(
        quantity.vx,
  ...
  const normalUnitVector = create$4$2();
  /**
   * Calculates and adds the impulsive force by the bounce.
   * Note that the penetration will not be fixed.
   */
  const addForceEachOther$1$1 = {
    calculate: (quantityA, quantityB, restitution) => {
      ...
      addForce$1(quantityB, bounceForce);
      multiply$1$1(bounceForce, -1);
      addForce$1(quantityA, bounceForce);
    },
    preCalculated: (
      quantityA,
      quantityB,
  ...
  const bounce = /*#__PURE__*/ Object.freeze({
    ...
    addForce: addForce$1$1,
    addForceEachOther: addForceEachOther$1$1,
   ...
   * @returns New array of HSV values.
   */
  const withRandomHue$1 = (saturation, value$1) => [
    value$4(360),
    saturation,
    value$1
  ];
  ...
  const CCC$1 = /*#__PURE__*/ Object.freeze({
    ...
    Angle: angle$5,
    ArrayList: index$1$2,
    ArrayQueue: index$2$1,
    Arrays: index$a,
    Bezier: bezier$1,
    Bounce: bounce,
    ConstantFunction: constantFunction$1,
    Coordinates2D: coordinates2d$1,
    Dynamics: dynamics$1,
    Easing: index$4$1,
    FitBox: fitBox$1,
  ...
  const {
    ...
    Gravitation,
    Bounce,
    Numeric: { square: square$3, max2: max2$3 }
  } = CCC$1;
  const {
    onSetup: onSetup$1,
    ShapeColor,
    drawTranslatedAndRotated: drawTranslatedAndRotated$1
  } = p5ex;
  /**
   * Shared p5 instance.
   */
   ...
   */
  const HTML_ELEMENT_ID = "NBodyProblem";
  /**
  ...
  const Body = (() => {
    ...
    let region;
    const reset = () => {
      region = RectangleRegion$1.createFromCenter(Vector2D$1.zero, {
        width: canvas$1.logicalSize.width - shellSize,
        height: canvas$1.logicalSize.height - shellSize
      });
    };
    ...
    const createTrailNode = body => ({
      x: body.x,
      y: body.y
    });
    const updateTrail = body => {
      const { trail } = body;
      ArrayQueue.dequeueIfFull(trail);
      ArrayQueue.enqueue(trail, createTrailNode(body));
    };
    const relativeVector = Vector2D$1.Mutable.create();
    const interact = (bodyA, bodyB) => {
      Vector2D$1.Assign.subtract(bodyB, bodyA, relativeVector);
      const distance = max2$3(shellSize, Vector2D$1.length(relativeVector));
    ...
    const drawCoreAtOrigin = () => {
      p$1.triangle(12, 0, -7, -8, -7, 8);
    };
    const draw = body => {
      const { x, y } = body;
      ShapeColor.apply(shellColor, 255);
      p$1.circle(x, y, shellSize);
      ShapeColor.apply(coreColor, 255);
      drawTranslatedAndRotated$1(
        drawCoreAtOrigin,
        x,
        y,
    ...
    const drawTrailNode = node => p$1.curveVertex(node.x, node.y);
    const drawTrail = body => {
      p$1.beginShape();
  ...
  const Bodies = (() => {
    ...
    const count = 16;
    const list = ArrayList$1.create(count);
    const reset = () => {
      ArrayList$1.clearReference(list);
      const maxBearing = 0.45 * canvas$1.logicalSize.height;
      for (let i = 0; i < count; i += 1) {
        const bearing = (1 - Random.Curved.ratio(square$3)) * maxBearing;
        ArrayList$1.add(list, Body.create(Random.vector(bearing)));
      }
    };
    const update = () => {
      ArrayList$1.loop(list, Body.preUpdate);
      ArrayList$1.roundRobin(list, Body.interact);
      ArrayList$1.loop(list, Body.postUpdate);
    };
    const draw = () => {
      ShapeColor.apply(trailColor, 255);
      ArrayList$1.loop(list, Body.drawTrail);
      ArrayList$1.loop(list, Body.draw);
    };
    return {
      reset,
      update,
  ...
  const updateSketch = () => {
    properFrameCount += 1;
    if (properFrameCount >= duration) reset$2();
    Bodies.update();
  };
  const drawSketch = () => {
    const { width, height } = canvas$1.logicalSize;
    ShapeColor.apply(timeBarColor, 255);
    p$1.rect(0, height - 10, (properFrameCount / duration) * width, height);
    const { x, y } = canvas$1.logicalCenterPosition;
    p$1.translate(x, y);
    Bodies.draw();
  };
  const draw$5 = () => {
