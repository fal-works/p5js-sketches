...
(function(p5) {
  ...
  const {
    ...
    Gravitation,
    Bounce,
    Numeric: { square: square$2, max2: max2$2 }
  } = CCC;
  const {
    onSetup: onSetup$1,
    ShapeColor,
  ...
  const Body = (() => {
    ...
    const interact = (bodyA, bodyB) => {
      ...
      const distance = max2$2(shellSize, Vector2D$1.length(relativeVector));
  ...
  const Bodies = (() => {
    ...
    const reset = () => {
      ...
      for (let i = 0; i < count; i += 1) {
        const bearing = (1 - Random.Curved.ratio(square$2)) * maxBearing;
