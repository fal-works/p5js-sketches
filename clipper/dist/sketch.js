/**
 * Clipper.
 * Source code in TypeScript: https://github.com/fal-works/p5js-sketches/tree/master/clipper
 *
 * Bundled libraries:
...
(function(p5) {
  ...
  const {
    Vector2D: Vector2D$1,
    Random,
    Numeric: { hypotenuse2D: hypotenuse2D$1, lerp: lerp$2 },
    Angle: { TWO_PI: TWO_PI$2, PI: PI$1 },
    Timer: Timer$1,
    Easing
  } = CCC;
  ...
  /** Shared canvas instance. */
  let renderer$1;
  const setRenderer$1 = r => {
    renderer$1 = r;
    setRenderer(r);
  };
  onSetup$1.push(setRenderer$1);

  /**
   * ---- Settings --------------------------------------------------------------
   */
  /** The id of the HTML element to which the canvas should belong. */
  const HTML_ELEMENT_ID = "Clipper";
  ...
  let diameter;
  const reset$2 = () => {
    const diagonalLength = hypotenuse2D$1(
      canvas$1.logicalSize.width,
      canvas$1.logicalSize.height
    );
  ...
  const createTimer = cone => {
    ...
    const wait = Timer$1.create({ duration: 90 });
  ...
  const create$g = (rotationAngle, rotationVelocity) => {
    const cone = {
      rotationVelocity,
      rotationAngle,
  ...
  const startPoint = Vector2D$1.Mutable.create();
  const drawShape = (startPoint, startAngle, endAngle) =>
    renderer$1.arc(
      startPoint.x,
      startPoint.y,
      diameter,
  ...
  const resetSurfaceGraphics = () => {
    ...
    g.translate(
      canvas$1.logicalCenterPosition.x,
      canvas$1.logicalCenterPosition.y
    );
    g.strokeWeight(4);
    surfaceGraphics = g;
  };
  const reset$3 = () => {
    p$1.image(coloredImage, 0, 0, p$1.width, p$1.height);
    drawBackground = storePixels();
    reset$2();
    cones.length = 0;
    const rotationAngleStep = TWO_PI$2 / 3;
    cones.push(
      create$g(0 * rotationAngleStep, 0.0005),
      create$g(1 * rotationAngleStep, -0.0005),
      create$g(2 * rotationAngleStep, 0.0015)
    );
  };
  const initialize = () => {
    reset$3();
  ...
  const updateSketch = () => {
    cones.forEach(update$2);
  };
  const updateSurface = () => {
    setRenderer$1(surfaceGraphics);
    renderer$1.erase();
    renderer$1.noStroke();
    renderer$1.fill(0);
    cones.forEach(draw$5);
    renderer$1.noErase();
    renderer$1.stroke(0, 160);
    renderer$1.noFill();
    cones.forEach(draw$5);
    setRenderer$1(p$1);
  };
  ...
  const setP5Methods = p => {
    p.preload = preload.bind(undefined, p);
