...
(function () {
    ...
    const sketch = (p) => {
        ...
        let hueRange;
        let alphaRange;
        let curveTightnessRange;
        // ---- functions
        function initialize() {
            p.background(backgroundColor);
            pointList = [];
            velocityList = [];
            pointCount = randomIntBetween(8, 24);
            for (let i = 0; i < pointCount; i++) {
                pointList.push(p.createVector(p.random(0.15, 0.85) * p.width, p.random(0.15, 0.85) * p.height));
        ...
        p.setup = () => {
            ...
            p.noFill();
            frameCounter = new NonLoopedFrameCounter(60, () => {
                p.noLoop();
            }).on();
            initialize();
        };
        p.draw = () => {
            ...
            const lightnessValue = p.lerp(lightnessRange.start, lightnessRange.end, progressRatio);
            const chromaValue = p.lerp(chromaRange.start, chromaRange.end, progressRatio);
            ...
            const curveColor = p.color(cielchColor(lightnessValue, chromaValue, hueValue, alphaValue));
            p.stroke(curveColor);
            const tightnessValue = p.lerp(curveTightnessRange.start, curveTightnessRange.end, progressRatio);
