/**
 * Contours 2.
 * Source code in TypeScript: https://github.com/fal-works/p5js-sketches/tree/master/contours-2
 *
 * Bundled libraries:
...
(function(p5) {
   ...
   */
  const HTML_ELEMENT_ID = "Contours2";
  /**
  ...
  const magnitude = 320;
  const tOffset = {
    x: Random$1.value(256),
    y: Random$1.value(256),
    s: Random$1.value(256),
    a: Random$1.value(256)
  };
  ...
  const update$3 = () => {
    t += 0.005;
    alphaT += 0.02;
    x = magnitude * (-0.5 + p$1.noise(tOffset.x + t));
    y = magnitude * (-0.5 + p$1.noise(tOffset.y + t));
    scaleFactor = 0.5 + p$1.noise(tOffset.s + t);
    const alphaNoise = p$1.noise(tOffset.a + alphaT);
  ...
  onSetup$1.push(p => {
    colorCandidates = ["#ef476f", "#ffd166", "#06d6a0", "#118ab2"]
      .map(code => reverseColor$1(p.color(code)))
  ...
  const create$9 = () => {
    ...
    return Object.assign(
      ...
      ),
      {
        ...
        color: Random$1.fromArray(colorCandidates),
        time: 0,
  ...
  const drawShape = (contour, scaleFactor) => {
    ...
    p$1.endShape();
    p$1.scale(1 / scaleFactor);
    p$1.rotate(-rotationAngle);
    p$1.translate(-x, -y);
  };
  const drawOutline = contour => {
    drawShape(contour, 1);
  };
  const drawShadow = contour => {
    ShapeColor.apply(contour.color, alpha);
    drawShape(contour, scaleFactor);
  };
  ...
  let outlineColor;
  onSetup$1.push(() => {
    outlineColor = ShapeColor.create(p$1.color(64), null, 256);
  });
  ...
  const update$5 = () => {
    ArrayList$1.loop(contours, update$4);
  ...
  const draw = () => {
    ...
    p$1.noStroke();
    p$1.translate(x, y);
    ArrayList$1.loop(contours, drawShadow);
    p$1.translate(-x, -y);
    p$1.blendMode(p$1.BLEND);
    ShapeColor.apply(outlineColor, 256 - alpha);
    p$1.strokeWeight(1);
    ArrayList$1.loop(contours, drawOutline);
  };
  const gather = () => {
