...
(function () {
    ...
    var ShapeType$1;
    (function (ShapeType$$1) {
        ShapeType$$1[ShapeType$$1["CIRCLE"] = 0] = "CIRCLE";
        ShapeType$$1[ShapeType$$1["HALF_CIRCLE"] = 1] = "HALF_CIRCLE";
        ShapeType$$1[ShapeType$$1["SQUARE"] = 2] = "SQUARE";
        ShapeType$$1[ShapeType$$1["TRIANGLE"] = 3] = "TRIANGLE";
        ShapeType$$1[ShapeType$$1["RECTANGLE"] = 4] = "RECTANGLE";
        ShapeType$$1[ShapeType$$1["L_SHAPE"] = 5] = "L_SHAPE";
        ShapeType$$1[ShapeType$$1["CROSS"] = 6] = "CROSS";
        ShapeType$$1[ShapeType$$1["RING"] = 7] = "RING";
        ShapeType$$1[ShapeType$$1["ARC"] = 8] = "ARC";
    ...
    class ShapeElement {
        constructor(p, position, color) {
            ...
            this.beginBirth = () => { this.birthTimer.on(); };
            this.die = () => { this.isToBeRemoved = true; };
            this.currentSize = 0;
            this.currentAngle = 0;
            this.shapeType = p.random(ShapeElement.typeList);
            this.birthDelayTimer = new NonLoopedFrameCounter(10, this.beginBirth).on();
            this.birthTimer = new NonLoopedFrameCounter(30).off();
            this.deathTimer = new NonLoopedFrameCounter(30, this.die).off();
            this.size = p.random(20, 50);
            this.rotationAngle = randomInt(8) * p.QUARTER_PI;
            this.rotationAngleFactor = Math.random() < 0.5 ? -1 : 1;
            switch (this.shapeType) {
                case ShapeType$1.CIRCLE:
                case ShapeType$1.HALF_CIRCLE:
                case ShapeType$1.SQUARE:
                case ShapeType$1.TRIANGLE:
                case ShapeType$1.RECTANGLE:
                    this.shapeColor = new ShapeColor(p, color, p.color(p.red(color), p.green(color), p.blue(color), 128));
                    break;
                case ShapeType$1.L_SHAPE:
                case ShapeType$1.CROSS:
                case ShapeType$1.RING:
                case ShapeType$1.ARC:
                    this.shapeColor = new ShapeColor(p, color, null);
                    break;
            }
        }
        step() {
            ...
            if (this.birthTimer.isOn) {
                ratio = easeOutCubic(this.birthTimer.getProgressRatio());
            }
            else if (this.deathTimer.isOn) {
                ratio = 1 - easeOutQuad(this.deathTimer.getProgressRatio());
            }
            else {
                if (this.beginDeath)
                    this.deathTimer.on();
    ...
    ShapeElement.typeList = [
        ShapeType$1.CIRCLE,
        ShapeType$1.HALF_CIRCLE,
        ShapeType$1.SQUARE,
    ...
    const sketch = (p) => {
        ...
        // ---- functions
        function generate() {
            shapeElements.loop((e) => { e.beginDeath = true; });
            const interval = p.nonScaledWidth / 4;
            for (let x = 1; x < 4; x += 1) {
                for (let y = 1; y < 4; y += 1) {
                    for (let i = 0; i < 3; i += 1) {
                        const position = p.createVector(interval * x + randomIntBetween(-3, 4) * 10, interval * y + randomIntBetween(-3, 4) * 10);
        ...
        p.setup = () => {
            ...
            colorList = [
                p.color(cielchColor(50, 90, p.radians(-90))),
                p.color(cielchColor(50, 90, p.radians(60))),
                p.color(cielchColor(80, 90, p.radians(90))),
            ];
            generate();
        };
        p.draw = () => {
