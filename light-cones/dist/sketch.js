...
(function(p5) {
   ...
   */
  const HTML_ELEMENT_ID = "LightCones";
  /**
  ...
  const createTimer = cone => {
    ...
    const move = Timer$1.create({
      ...
      onProgress: listener => {
        ...
        cone.saturation = lerp$2(startSaturation, endSaturation, ratio);
  ...
  const create$g = (rotationAngle, hue, rotationVelocity) => {
    const cone = {
      rotationVelocity,
      rotationAngle,
  ...
  const draw$5 = cone => {
    p$1.fill(cone.hue, cone.saturation, cone.brightness);
  ...
  const reset$3 = () => {
    ...
    const hueOffset = p$1.random(60);
    cones.push(
      create$g(0 * rotationAngleStep, hueOffset, 0.001),
      create$g(1 * rotationAngleStep, (hueOffset + 150) % 360, -0.001),
      create$g(2 * rotationAngleStep, (hueOffset + 210) % 360, 0.003)
    );
  };
  const initialize = () => {
    p$1.noStroke();
    p$1.blendMode(p$1.HARD_LIGHT);
    p$1.colorMode(p$1.HSB, 360, 100, 100, 100);
    reset$3();
  };
  const updateSketch = () => {
