...
const sketch = (p) => {
    ...
    const canvasRegion = new RectangleRegion(0, 0, 640, 640);
    const worldRegion = new RectangleRegion(40, 120, 600, 600);
    const regionColor = new ShapeColor(p, p.color(0, 24), p.color(252));
    const cursorColor = new ShapeColor(p, null, p.color(0, 64));
    const cursorSize = 24;
    const cursorTransformFactor = 0.3;
    // ---- variables
    const entityPool = new SpriteArray(256);
    const aliveEntities = new CleanableSpriteArray(256);
    const mousePosition = p.createVector();
    const globalTimer = new LoopedFrameCounter(60);
    // ---- functions
    ...
    p.setup = () => {
        ...
        Entity.initialize(entityPool, worldRegion, new ShapeColor(p, null, p.color(64), true), new ShapeColor(p, p.color(128), null, true));
...
class Entity {
    constructor(p) {
        ...
        this.isAlive = false;
        this.birthTimer = new NonLoopedFrameCounter(15).off();
        this.deathTimer = new NonLoopedFrameCounter(15).off();
        this.coolingTimer = new NonLoopedFrameCounter(60).off();
        this.timers = new SteppableArray(3);
        ...
        const properColor = subtractColor(p.color(255), p.color(cielchColor(80, 100, p.random(0, p.TWO_PI))));
        this.properShapeColor = new ShapeColor(p, null, properColor);
        this.properLineColor = new ShapeColor(p, properColor, undefined, true);
    ...
    drawBirthLine() {
        ...
        this.drawTrimmedLine(this.waitingPosition, this.position, easeLinear(ratio), easeOutQuart(ratio));
    }
    drawDeathLine() {
        const ratio = this.deathTimer.getProgressRatio();
        this.p.blendMode(this.p.BLEND);
        Entity.deadLineColor.applyColor(64 + ratio * 191);
        this.drawTrimmedLine(this.deathPosition, this.waitingPosition, easeLinear(ratio), easeOutQuart(ratio));
    }
}
