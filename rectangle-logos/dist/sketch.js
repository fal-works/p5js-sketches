...
if (OPENPROCESSING)
    new p5();
class Logo {
    constructor(p, size, x, y) {
        this.p = p;
        this.size = size;
        this.position = p.createVector(x + 0.5 * size, y + 0.5 * size);
        this.reset();
        this.appearanceTimer = new NonLoopedFrameCounter(30, () => {
            this.pauseTimer.resetCount().on();
        });
        this.pauseTimer = new NonLoopedFrameCounter(randomIntBetween(60, 180), () => {
            this.disappearanceTimer.resetCount().on();
        }).off();
        this.disappearanceTimer = new NonLoopedFrameCounter(30, () => {
    ...
    reset() {
        ...
        this.color = new ShapeColor(p, null, p.color(p.random(['#000d1a', '#003070', '#808080'])));
        const data = [];
    ...
    draw() {
        ...
        const scaleFactor = this.appearanceTimer.isOn ?
            easeOutQuart(this.appearanceTimer.getProgressRatio()) :
