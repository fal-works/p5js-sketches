...
(function (p5) {
  ...
  p5 =
    p5 && Object.prototype.hasOwnProperty.call(p5, "default")
      ? p5["default"]
      : p5;
   ...
   * @param resolution
   */
  const create$h = (color, resolution) => {
  ...
  const overwrite = (shapeColor, strokeColor, fillColor, alphaResolution) => {
    ...
    if (strokeColor === null) {
      ...
      const strokeAlphaColor = create$h(strokeColor, alphaResolution);
    ...
    if (fillColor === null) {
      ...
      const fillAlphaColor = create$h(fillColor, alphaResolution);
  ...
  const HTML_ELEMENT_ID = "UnaryNumeralSystem";
  ...
  const candidates = [syou, tally, truco];

  const create$i = (y) => ({
    y,
    time: Math.random() * 1024,
    letter: p$1.random(candidates),
  });
  const update$2 = (line) => {
    line.time += 0.02;
  };
  ...
  let maxCellCount = 1;
  const updateStatic = () => {
    maxCellCount = canvas$1.logicalSize.width / cellSize;
  };
  const draw$5 = (line) => {
    const value = Math.floor(p$1.noise(line.time) * maxCellCount * 5);
    ...
    for (let i = 0; i < fullCellCount; i += 1) {
      translate$1(i * cellSize, line.y);
      letter.forEach((f) => f());
      undoTranslate$1();
    }
    translate$1(fullCellCount * cellSize, line.y);
  ...
  const reset$2 = () => {
    ...
    canvas$1.drawScaled(() => {
      ...
      p$1.background(248);
      p$1.stroke(128, 4);
      p$1.strokeCap(p$1.SQUARE);
      p$1.strokeWeight(2);
      for (let i = 0; i < 5000; i += 1) {
        const x = p$1.random(0, width);
        const y = p$1.random(-50, height - 50);
        p$1.line(x, y, x, y + p$1.random(50, 100));
      }
    });
    drawBackground = storePixels();
    p$1.strokeCap(p$1.ROUND);
  };
  const initialize = () => {
    reset$2();
    p$1.stroke(0);
    p$1.strokeWeight(2);
    lines = [];
    for (let y = 0; y < canvas$1.logicalSize.height - cellSize; y += cellSize) {
      lines.push(create$i(y));
    }
  };
  const updateSketch = () => {
    updateStatic();
    lines.forEach(update$2);
  };
  const drawSketch = () => {
    lines.forEach(draw$5);
  };
  const draw$6 = () => {
    updateSketch();
    drawBackground();
  ...
  startSketch({
    ...
    windowResized: () => canvas$1.resizeIfNeeded(),
    p5Methods: p5Methods,
  });
})(p5);
