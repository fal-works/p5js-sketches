...
const sketch = (p) => {
    ...
    let timeoutId = -1;
    let firstArcRotationAngle;
    let firstArcEndAngle;
    let secondArcRotationAngle;
    let secondArcEndAngle;
    let linesRotationAngle;
    let firstLineStartX;
    let firstLineEndX;
    let firstLineY;
    let secondLineStartX;
    let secondLineEndX;
    let secondLineY;
    // ---- functions
    ...
    p.draw = () => {
        if (resetIndicator || resizeIndicator) {
            ...
            applyRandomTexture(p, 32, true, 192, 192, 255);
            p.loadPixels();
